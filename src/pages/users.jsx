...
function ManageRequestPage() {
        ...
        const fetchRequests = async () => {
            try {
                ...
                const [response, response2, response3, response4] = await Promise.all([
                    fetch(`https://localhost:7097/challenger?AccountID=${user.sub}`, {
                        headers: { 'Authorization': `Bearer ${accessToken}` }
                    }),
                    fetch(`https://localhost:7097/defender?AccountID=${user.sub}`, {
                        headers: { 'Authorization': `Bearer ${accessToken}` }
                    }),
                    fetch(`https://localhost:7097/ongoing?AccountID=${user.sub}`, {
                        headers: { 'Authorization': `Bearer ${accessToken}` }
                    }),
                    fetch(`https://localhost:7097/complete?AccountID=${user.sub}`, {
                        headers: { 'Authorization': `Bearer ${accessToken}` }
                    })
                ]);
    ...
    const handleUpdate = async (request) => {
        try {
            ...
            const accessToken = await getAccessTokenSilently();
            const response = await fetch(`https://localhost:7097/api/Values/update/${request.id}`, {
                method: 'PUT', 
                headers: {
